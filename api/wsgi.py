#!/usr/bin/env python3
"""
WSGI entry point for the Little Wins API
...
if __name__ == "__main__":
    ...
    if env == "production":
        ...
        print(f"Starting Little Wins API with Gunicorn on port {port}")
        ...
        print(f"Starting Little Wins API (development) on port {port}")
        print(f"Environment: {env}")
        print(
            f"Google Client ID: {'Set' if app.config.get('GOOGLE_CLIENT_ID') else 'Missing'}"
        )
